...
function ageClassification(n) {
  var res;
  (n < 0 || n > 122) ? res = null : n <= 24 ? res = 'детский возраст': n <= 44 ? res = 'молодой возраст': n <= 65 ? res = 'средний возраст':
  n <= 75 ? res = 'пожилой возраст': (n <= 122 && n > 90) ? res = 'долгожители':  res = 'старческий возраст';
  return res;
};
...
function mainFunc(a, b, func) {
  var res;
  if ((typeof arguments[0] !== 'number') || (typeof arguments[1] !== 'number') || (typeof arguments[2] !== 'function')) {res = false}
  else res = func(a, b);
  return res;
...
function cbRandom(a, b) {
  var res = 0;
  if ((a < 1) || (a >= b)) res = 'Некорректные значения'
  else {
    while ((res < a) || (res > b)) res = Math.trunc((Math.random() * (Math.pow(10, (Math.trunc(b).toString().length)))))
  };
  return res;
}
...
function cbPow(a, b) {
  var res;
  if ((Number.isInteger(a)) && (Number.isInteger(b))) {res = Math.pow(a, b)}
  else res = 'Некорректные значения';
  return res;
 };
...
console.log(mainFunc(2, 5, 'not a func')); // false
console.log(mainFunc(5, 5, cbRandom));
